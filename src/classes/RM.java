...
public class RM implements Escalonador {
    PriorityQueue<InstanciaTarefa> filaPronto = new PriorityQueue<>(Comparator.comparingInt(inst -> inst.getTarefa().getPeriodo()));
    ...
    public RM(List<Tarefa> tarefasOriginais) {
    ...
    public boolean testeEscalonabilidade(List<Tarefa> tarefas) {
        int n = tarefas.size();
        double limite = n * (Math.pow(2, 1.0 / n) - 1);
        double utilizacao = calcularUtilizacao(tarefas);
        System.out.printf("Utilização = %.2f, Limite RM = %.2f%n\n", utilizacao, limite);
    ...
    public void simular(List<Tarefa> listaTarefas, int tempoSim) {
        ...
        if (testeEscalonabilidade(tarefasOriginais)) {
            System.out.println("O teste de escalonabilidade é SUFICIENTE\n");
        } else {
        ...
        do {
            ...
            for (Tarefa t : tarefasOriginais) {
                if ((instante - t.getOffset()) % t.getPeriodo() == 0 && instante >= t.getOffset()) {
                    ...
                    if (instante == t.getOffset()) {
                        System.out.println("Instante " + instante + ": Tarefa ID " + nova.getIdInstancia() +
                                " liberada (Offset inicial = " + t.getOffset() + ")");
                    } else {
                        System.out.println("Instante " + instante + ": Tarefa ID " + nova.getIdInstancia() +
                                " liberada (Periódica)");
                    }

                    //Detecção de inversão de prioridade
                    if (!processador.isEmpty()) {
                        InstanciaTarefa emExecucao = processador.get(0);
                        if (nova.getTarefa().getPeriodo() < emExecucao.getTarefa().getPeriodo()) {
                            System.out.printf("Inversão de prioridade no instante %d: Tarefa ID %d (prioridade maior) ficou esperando enquanto ID %d executava%n",
                                    instante, nova.getIdInstancia(), emExecucao.getIdInstancia());
                        }
                    }
                }
            }
            ...
            if (!processador.isEmpty()) {
                ...
                InstanciaTarefa maisPrioritaria = filaPronto.peek();

                if (maisPrioritaria != null &&
                        maisPrioritaria.getTarefa().getPeriodo() < atual.getTarefa().getPeriodo()) {
            ...
            //Se processador vazio, coloca a mais prioritária
            ...
            //Executa o que está no processador
            if (!processador.isEmpty()) {
                InstanciaTarefa atual = processador.get(0);
                atual.processa();
            ...
            //Verificação de deadline perdido
            List<InstanciaTarefa> perdidas = new ArrayList<>();
            for (InstanciaTarefa inst : filaPronto) {
                int deadline = inst.getInstanteChegada() + inst.getTarefa().getDeadline();
                if (instante >= deadline) {
                    System.out.printf("Tarefa ID %d perdeu o deadline no instante %d (Deadline = %d)%n",
                            inst.getIdInstancia(), instante, deadline);
    ...
    public void mostraFilaPronto() {
        ...
        lista.sort(Comparator.comparingInt(inst -> inst.getTarefa().getPeriodo()));
